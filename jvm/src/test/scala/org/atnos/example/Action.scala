...
trait ActionImplicits extends ActionTypes {
  ...
  implicit def ErrorOrOkMember: Member.Aux[ErrorOrOk, ActionStack, In3[Console, Warnings, Eval]] =
    Member.Member4L[ErrorOrOk, Console, Warnings, Eval]

  implicit def ConsoleMember =
    Member.MemberAppendR[Console, In1[ErrorOrOk], In3[Console, Warnings, Eval], In2[Warnings, Eval]](Member.Member3L[Console, Warnings, Eval])

  implicit def WarningsMember =
    Member.MemberAppendR[Warnings, In1[ErrorOrOk], In3[Console, Warnings, Eval], In2[Console, Eval]](Member.Member3M[Console, Warnings, Eval])

  implicit def EvalMember =
    Member.MemberAppendR[Eval, In1[ErrorOrOk], In3[Console, Warnings, Eval], In2[Console, Warnings]](Member.Member3R[Console, Warnings, Eval])
}
